...
export async function submitContactForm(
  ...
  const to = process.env.CONTACT_TO_EMAIL || "proclean.caen@hotmail.com";
  const from = process.env.CONTACT_FROM_EMAIL || "PROCLEAN STJ <onboarding@resend.dev>";
  ...
  const html = `
    <h2>Nouvelle demande de devis — PROCLEAN STJ</h2>
    <ul>
      ...
      <li><strong>Prestation :</strong> ${escapeHtml(service) || "—"}</li>
  ...
  try {
    ...
    if (!response.ok) {
      ...
      return {
        status: "error",
        message:
          "Une erreur est survenue lors de l'envoi. Vous pouvez nous écrire directement à proclean.caen@hotmail.com.",
      };
    }
  } catch (error) {
